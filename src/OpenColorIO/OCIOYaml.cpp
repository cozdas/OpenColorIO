...
// Basic types
 ...
 void load(const YAML::Node& node, bool& x)
{
    try
    {
        x = node.as<bool>();
 ...
 void load(const YAML::Node& node, double& x)
{
    try
    {
        x = node.as<double>();
 ...
 void load(const YAML::Node& node, std::string& x)
{
 ...
 const std::string load_str(const YAML::Node& node)
 {
	 try
	 {
 ...
 void load(const YAML::Node & node, StringUtils::StringVec & x)
{
    try
    {
 ...
 void load(const YAML::Node & node, std::vector<float> & x)
{
    try
    {
 ...
 void load(const YAML::Node& node, std::vector<double>& x)
{
    try
    {
...
// Enums
 ...
 void load(const YAML::Node& node, BitDepth& depth)
{
    std::string str;
    load(node, str);
...
//  void save(YAML::Emitter& out, BitDepth depth)
// {
//     out << BitDepthToString(depth);
// }

 void load(const YAML::Node& node, Allocation& alloc)
{
    std::string str;
    load(node, str);
...
//  void save(YAML::Emitter& out, Allocation alloc)
// {
...
//  void save(YAML::Emitter& out, TransformDirection dir)
// {
//     out << TransformDirectionToString(dir);
// }

 void load(const YAML::Node& node, Interpolation& interp)
{
    std::string str;
    load(node, str);
...
//  void save(YAML::Emitter& out, Interpolation interp)
// {
//     out << InterpolationToString(interp);
// }

 void loadDescription(const YAML::Node& node, std::string& x)
{
    load(node, x);
    if (!x.empty())
    {
...
//  void saveDescription(YAML::Emitter & out, const char * desc)
// {
//     if (desc && *desc)
//     {
...
//
 ...
 void LogUnknownKeyWarning(const YAML::Node & node,
                                 const YAML::Node & key)
{
    std::string keyName;
    load(key, keyName);
 ...
 void LogUnknownKeyWarning(const std::string & name,
                                 const YAML::Node & tag)
{
    std::string key;
    load(tag, key);
 ...
 void throwError(const YAML::Node& node,
	 const char* msg)
 {
	 std::ostringstream os;
	 os << "At line " << (node.Mark().line + 1)
...
void throwValueError(const std::string& nodeName,
 ...
 void throwValueError(const YAML::Node & key,
                            const std::string& msg)
{
    std::string keyName;
    load(key, keyName);
 ...
 void CheckDuplicates(const YAML::Node & node)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
    {
        ...
        else
        {
            ...
            throwValueError(node.Tag(), iter->first, os);
        }
    }
}
...
// View
 ...
 void load(const YAML::Node& node, View& v)
{
    if(node.Tag() != "View")
        return;
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
    {
        const auto& iter_second = iter->second;
        const std::string & key = iter->first.as<std::string>();

        if (iter_second.IsNull() || !iter_second.IsDefined()) continue;

        if(key == "name")
        {
            load(iter_second, v.m_name);
        }
        else if (key == "view_transform")
        {
            expectingDisplayCS = true;
            load(iter_second, v.m_viewTransform);
        }
        else if(key == "colorspace")
        {
            expectingSceneCS = true;
            load(iter_second, v.m_colorspace);
        }
        ...
        {
            load(iter_second, v.m_looks);
        }
        else if (key == "rule")
        {
            load(iter_second, v.m_rule);
        }
        else if (key == "description")
        {
            load(iter_second, v.m_description);
        }
        else
        {
...
//  void save(YAML::Emitter& out, const View & view)
// {
...
// }
// 
//  void EmitTransformName(YAML::Emitter & out,
...
// AllocationTransform
 ...
 void load(const YAML::Node& node, AllocationTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstAllocationTransformRcPtr t)
// {
...
// BuiltinTransform
 ...
 void load(const YAML::Node & node, BuiltinTransformRcPtr & t)
{
...
//  void save(YAML::Emitter & out, const ConstBuiltinTransformRcPtr & t)
// {
...
// CDLTransform
 ...
 void load(const YAML::Node& node, CDLTransformRcPtr& t)
{
 ...
 void save(YAML::Emitter& out, ConstCDLTransformRcPtr t, unsigned int majorVersion)
{
...
// ColorSpaceTransform
 ...
 void load(const YAML::Node& node, ColorSpaceTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstColorSpaceTransformRcPtr t)
// {
...
// DisplayViewTransform
 ...
 void load(const YAML::Node& node, DisplayViewTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstDisplayViewTransformRcPtr t)
// {
...
// ExponentTransform
 ...
 void load(const YAML::Node& node, ExponentTransformRcPtr& t)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
    {
        ...
        if (key == "value")
        {
            ...
            if (iter_second.Type() == YAML::NodeType::Sequence)
            {
                load(iter_second, val);
            }
            else
...
//  void save(YAML::Emitter& out, ConstExponentTransformRcPtr t, unsigned int majorVersion)
// {
...
// ExponentWithLinear
 ...
 void load(const YAML::Node& node, ExponentWithLinearTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstExponentWithLinearTransformRcPtr t)
// {
...
// ExposureContrastTransform
 ...
 void load(const YAML::Node& node, ExposureContrastTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstExposureContrastTransformRcPtr t)
// {
...
// FileTransform
 ...
 void load(const YAML::Node& node, FileTransformRcPtr& t)
{
 ...
 void save(YAML::Emitter& out, ConstFileTransformRcPtr t, unsigned int majorVersion)
{
...
// FixedFunctionTransform
 ...
 void load(const YAML::Node& node, FixedFunctionTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstFixedFunctionTransformRcPtr t)
// {
...
// GradingPrimaryTransform
 ...
 void load(const YAML::Node & parent, const YAML::Node & node, GradingRGBM & rgbm)
{
    if (node.Type() == YAML::NodeType::Map)
    {
        ...
        for (Iterator iter = node.begin(); iter != node.end(); ++iter)
		{
            ...
            {
                load(iter_second, rgbm.m_master);
                masterOK = true;
            }
            else
            {
 ...
 void loadPivot(const YAML::Node & parent, const YAML::Node & node,
                      ...
                      double & whiteVal, bool & whiteValLoaded)
{
    if (node.Type() == YAML::NodeType::Map)
    {
        for (Iterator iter = node.begin(); iter != node.end(); ++iter)
		{
            ...
            {
                load(iter_second, blackVal);
                blackValLoaded = true;
            }
            else if (key == "white")
            {
                load(iter_second, whiteVal);
                whiteValLoaded = true;
            }
            else
            {
 ...
 void loadClamp(const YAML::Node & parent, const YAML::Node & node,
                      double & blackVal, bool & blackValLoaded,
                      double & whiteVal, bool & whiteValLoaded)
{
    if (node.Type() == YAML::NodeType::Map)
 ...
 void load(const YAML::Node & node, GradingPrimaryTransformRcPtr & t)
{
    CheckDuplicates(node);
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        else if (key == "brightness")
        {
            ...
            load(iter->first, iter_second, values.m_brightness);
        }
        else if (key == "contrast")
        {
            contrastLoaded = true;
            load(iter->first, iter_second, values.m_contrast);
        }
        else if (key == "gamma")
        {
            gammaLoaded = true;
            load(iter->first, iter_second, values.m_gamma);
        }
        else if (key == "offset")
        {
            offsetLoaded = true;
        ...
        {
            exposureLoaded = true;
            load(iter->first, iter_second, values.m_exposure);
        }
        else if (key == "lift")
        {
            liftLoaded = true;
            load(iter->first, iter_second, values.m_lift);
        }
        else if (key == "gain")
        {
            gainLoaded = true;
            load(iter->first, iter_second, values.m_gain);
        }
        else if (key == "pivot")
        {
            loadPivot(iter->first, iter_second, values.m_pivot, pivotLoaded,
                      values.m_pivotBlack, pivotBlackLoaded, values.m_pivotWhite, pivotWhiteLoaded);
        }
        else if (key == "saturation")
        {
            saturationLoaded = true;
            load(iter_second, values.m_saturation);
        }
        else if (key == "clamp")
        {
            loadClamp(iter->first, iter_second, values.m_clampBlack, clampBlackLoaded,
...
//  void save(YAML::Emitter & out, const char * paramName, const GradingRGBM & rgbm,
...
//  void save(YAML::Emitter & out, const char * paramName, double val, double defaultVal)
// {
//     if (val != defaultVal)
//     {
...
//  void savePivot(YAML::Emitter & out, double val, bool saveContrast,
...
// }
// 
//  void saveClamp(YAML::Emitter & out,
//                       double blackVal, double defaultBlackVal,
//                       double whiteVal, double defaultWhiteVal)
// {
...
//  void save(YAML::Emitter & out, ConstGradingPrimaryTransformRcPtr t)
// {
...
// GradingRGBCurveTransform
 ...
 void load(const YAML::Node & parent, const YAML::Node & node, GradingBSplineCurveRcPtr & sc)
{
    if (node.Type() == YAML::NodeType::Map)
    {
        bool cpOK{ false };
 ...
 void load(const YAML::Node & node, GradingRGBCurveTransformRcPtr & t)
{
    CheckDuplicates(node);
...
//  void save(YAML::Emitter & out, const char * paramName, const ConstGradingBSplineCurveRcPtr & curve)
// {
...
//  void save(YAML::Emitter & out, ConstGradingRGBCurveTransformRcPtr t)
// {
//     const auto & vals = t->getValue();
//     auto & defCurve = t->getStyle() == GRADING_LIN ? GradingRGBCurveImpl::DefaultLin :
//                                                      GradingRGBCurveImpl::Default;
...
// GradingToneTransform
 ...
 void load(const YAML::Node & parent, const YAML::Node & node, GradingRGBMSW & rgbm,
                 bool center, bool pivot)
{
    if (node.Type() == YAML::NodeType::Map)
    {
        ...
        for (Iterator iter = node.begin(); iter != node.end(); ++iter)
		{
            ...
            {
                load(iter_second, rgbm.m_start);
                startOK = true;
            }
            else if (key == (pivot ? "pivot" : "width"))
            {
                load(iter_second, rgbm.m_width);
                widthOK = true;
            }
            else
            {
 ...
 void load(const YAML::Node & node, GradingToneTransformRcPtr & t)
{
    CheckDuplicates(node);
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        {
            blacksLoaded = true;
            load(iter->first, iter_second, blacks, false, false);
        }
        else if (key == "shadows")
        {
            shadowsLoaded = true;
            load(iter->first, iter_second, shadows, false, true);
        }
        else if (key == "midtones")
        {
            midtonesLoaded = true;
            load(iter->first, iter_second, midtones, true, false);
        }
        else if (key == "highlights")
        {
            highlightsLoaded = true;
            load(iter->first, iter_second, highlights, false, true);
        }
        else if (key == "whites")
        {
            whitesLoaded = true;
            load(iter->first, iter_second, whites, false, false);
        }
        else if (key == "s_contrast")
        {
            load(iter_second, scontrast);
        }
        else if (key == "name")
        {
            std::string name;
...
//  void save(YAML::Emitter & out, const char * paramName, const GradingRGBMSW & rgbm,
...
//  void save(YAML::Emitter & out, ConstGradingToneTransformRcPtr t)
// {
...
void save(YAML::Emitter& out, ConstTransformRcPtr t, unsigned int majorVersion);
 ...
 void load(const YAML::Node& node, GroupTransformRcPtr& t)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        if(key == "children")
        {
            for (std::size_t i = 0; i < iter_second.size(); i++)
            {
                const YAML::Node & val = iter_second[i];
...
//  void save(YAML::Emitter& out, ConstGroupTransformRcPtr t, unsigned int majorVersion)
// {
...
// LogAffineTransform
 ...
 void loadLogParam(const YAML::Node & node,
                         double(&param)[3],
                         const std::string & paramName)
{
 ...
 void load(const YAML::Node& node, LogAffineTransformRcPtr& t)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        if (key == "base")
        {
            size_t nb = iter_second.size();
            if (nb == 0)
            {
                load(iter_second, base);
            }
            else
            {
                std::ostringstream os;
        ...
        {
            loadLogParam(iter_second, linOffset, key);
        }
        else if (key == "lin_side_slope")
        {
            loadLogParam(iter_second, linSlope, key);
        }
        else if (key == "log_side_offset")
        {
            loadLogParam(iter_second, logOffset, key);
        }
        else if (key == "log_side_slope")
        {
            loadLogParam(iter_second, logSlope, key);
        }
        else if(key == "direction")
        {
            TransformDirection val;
...
//  void saveLogParam(YAML::Emitter& out, const double(&param)[3],
...
//  void save(YAML::Emitter& out, ConstLogAffineTransformRcPtr t)
// {
...
// LogCameraTransform
 ...
 void load(const YAML::Node & node, LogCameraTransformRcPtr & t)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        {
            linBreakFound = true;
            loadLogParam(iter_second, linBreak, key);
        }
        else if (key == "linear_slope")
        {
            linearSlopeFound = true;
            loadLogParam(iter_second, linearSlope, key);
        }
        else if (key == "direction")
        {
            TransformDirection val;
...
//  void save(YAML::Emitter& out, ConstLogCameraTransformRcPtr t)
// {
...
// LogTransform
 ...
 void load(const YAML::Node& node, LogTransformRcPtr& t)
{
...
//             t->getFormatMetadata().setName(name.c_str());
            ...
            tp->getFormatMetadata().setName(load_str(iter_second));
...
//  void save(YAML::Emitter& out, ConstLogTransformRcPtr t, unsigned int majorVersion)
// {
...
// LookTransform
 ...
 void load(const YAML::Node& node, LookTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstLookTransformRcPtr t)
// {
...
// MatrixTransform
 ...
 void load(const YAML::Node& node, MatrixTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstMatrixTransformRcPtr t, unsigned int majorVersion)
// {
...
// RangeTransform
 ...
 void load(const YAML::Node& node, RangeTransformRcPtr& t)
{
...
//  void save(YAML::Emitter& out, ConstRangeTransformRcPtr t)
// {
...
TransformRcPtr load_transform(const YAML::Node& node)
 {
    TransformRcPtr t;
    load(node, t);
    return t;
...
// ColorSpace
 ...
 void load(const YAML::Node& node, ColorSpaceRcPtr& cs_sp, unsigned int majorVersion)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        {
            loadDescription(iter_second, stringval);
...
//             cs->setTransform(val, COLORSPACE_DIR_TO_REFERENCE);
            ...
            cs->setTransform(load_transform(iter_second), COLORSPACE_DIR_TO_REFERENCE);
...
//             load(iter_second, val);
//             cs->setTransform(val, COLORSPACE_DIR_FROM_REFERENCE);
            cs->setTransform(load_transform(iter_second), COLORSPACE_DIR_FROM_REFERENCE);
...
//  void save(YAML::Emitter& out, ConstColorSpaceRcPtr cs, unsigned int majorVersion)
// {
...
// Look
 ...
 void load(const YAML::Node& node, LookRcPtr& look)
{
    if(node.Tag() != "Look")
        return;
...
//  void save(YAML::Emitter& out, ConstLookRcPtr look, unsigned int majorVersion)
// {
...
// View transform
 ...
 ReferenceSpaceType peekViewTransformReferenceSpace(const YAML::Node & node)
{
    if (node.Type() != YAML::NodeType::Map)
    {
 ...
 void load(const YAML::Node & node, ViewTransformRcPtr & vt)
{
...
//  void save(YAML::Emitter & out, ConstViewTransformRcPtr & vt, unsigned int majorVersion)
// {
...
// NamedTransform
 ...
 void load(const YAML::Node & node, NamedTransformRcPtr & nt)
{
...
//  void save(YAML::Emitter & out, ConstNamedTransformRcPtr & nt, unsigned int majorVersion)
// {
...
struct CustomKeysLoader
{
 ...
 void loadCustomKeys(const YAML::Node& node, CustomKeysLoader & ck)
{
    if (node.Type() == YAML::NodeType::Map)
    {
        for (Iterator iter = node.begin(); iter != node.end(); ++iter)
		{
            ...
            const std::string & key = iter->first.as<std::string>();
            const std::string & val = iter_second.as<std::string>();
 ...
 void load(const YAML::Node & node, FileRulesRcPtr & fr, bool & defaultRuleFound)
{
    if (node.Tag() != "Rule")
        return;
...
//  void save(YAML::Emitter & out, ConstFileRulesRcPtr & fr, size_t position)
// {
...
// Viewing rules
 ...
 void load(const YAML::Node & node, ViewingRulesRcPtr & vr)
{
    if (node.Tag() != "Rule")
        return;
...
//  void save(YAML::Emitter & out, ConstViewingRulesRcPtr & vr, size_t position)
// {
//     out << YAML::VerbatimTag("Rule");
//     out << YAML::Flow;
//     out << YAML::BeginMap;
...
// Config
 ...
 void load(const YAML::Node& node, ConfigRcPtr & config, const char* filename)
{
    ...
    for (Iterator iter = node.begin(); iter != node.end(); ++iter)
	{
        ...
        else if(key == "environment")
        {
            ...
            if(iter_second.Type() != YAML::NodeType::Map)
            {
                throwValueError(node.Tag(), iter->first, 
                                "The value type of key 'environment' needs to be a map.");
            }
            for (Iterator it = iter_second.begin(); it != iter_second.end(); ++it)
            {
        ...
        else if (key == "file_rules")
        {
            ...
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "The 'file_rules' field needs to be a (- !<Rule>) list.");
            }
            ...
            for (std::size_t i = 0; i < iter_second.size(); i++)
            {
                ...
                if (val.Tag() == "Rule")
                {
                    ...
                    {
                        throwError(iter_second, "The 'file_rules' Default rule has to be "
                                           "the last rule.");
                    }
        ...
        else if (key == "viewing_rules")
        {
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "The 'viewing_rules' field needs to be a (- !<Rule>) list.");
            }
        ...
        else if(key == "colorspaces")
        {
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "'colorspaces' field needs to be a (- !<ColorSpace>) list.");
            }
        ...
        else if (key == "display_colorspaces")
        {
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "'display_colorspaces' field needs to be a (- !<ColorSpace>) list.");
            }
        ...
        else if (key == "looks")
        {
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "'looks' field needs to be a (- !<Look>) list.");
            }
        ...
        else if (key == "view_transforms")
        {
            if (iter_second.Type() != YAML::NodeType::Sequence)
            {
                throwError(iter_second, "'view_transforms' field needs to be a (- !<ViewTransform>) list.");
            }
...
//  void save(YAML::Emitter & out, const Config & config)
// {
//     std::stringstream ss;
